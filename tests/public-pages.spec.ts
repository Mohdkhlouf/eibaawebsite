...
test.describe('Public Pages', () => {
   test('blogs page loads and displays content', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/مقالات الاخصائية الأسرية إباء أبو طه/)
  ...
  test('services page loads', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/موقع الاخصائية الأسرية إباء أبو طه - الخدمات/)
  })

  test('courses page loads', async ({ page }) => {
    await page.goto('http://localhost:3000/courses')

    await expect(page).toHaveTitle(/موقع الاخصائية الأسرية إباء أبو طه - الدورات/)
  })
